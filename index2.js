...
async function askGPTBounded(prompt, maxChars = MAX_CHARS) {
  ...
  // Meta-context to apply to the query
  const system = [
  ...
  const user = [
    ...
    'Unicode emojis are acceptable, but only as an afterthought, if there is room and if it is wonderfully relevant; keep it short.',
    '',
    `Question: ${prompt}`,
  ].join('\n');
...
async function ensureAuth() {
  ...
  app.get('/oauth2callback', async (req, res) => {
    try {
      ...
      oauth2Client.setCredentials(tokens);
      fs.writeFileSync(TOKEN_PATH, JSON.stringify(tokens, null, 2)); // overwrites if exists
  ...
  if (MODE === 'dev') {
    // Load dev state early so the Dev Panel shows the cached chatId immediately
+   loadDevState();

    function devHtml(status) {
      const s = JSON.stringify(status || {}, null, 2);
      const stateExists = fs.existsSync(DEV_STATE_PATH);
      return `
        <!doctype html>
        <meta charset="utf-8" />
        ...
        <div class="state-status">
          State file: 
          <span class="${stateExists ? 'exists' : 'missing'}">
            ${stateExists ? '✅ Present' : '❌ Missing'}
          </span>
          (${DEV_STATE_PATH})
        </div>
    ...
    function ensureDev(app) {
      ...
      function devHtml(status) {
        ...
        return `
          ...
          <div class="row">
            ...
            <form action="/dev/poll"    method="post"><button>3) Poll once</button></form>
            <form action="/dev/whoami"  method="post"><button>Who am I?</button></form>
            <form action="/dev/reset"   method="post" onsubmit="return confirm('Delete dev_state.json and clear memory?')">
      ...
      app.get('/dev', (req, res) =>
        res.send(devHtml({ liveChatId: g.liveChatId, nextPageToken: g.nextPageToken, primed: g.primed }))
      );

      app.post('/dev/connect', async (req, res) => {
        try {
          // DEV: force livestream URL; prefer cached state to avoid calls
          if (!TARGET_LIVESTREAM_URL) {
            return res.send(devHtml({ error: 'Set TARGET_LIVESTREAM_URL in .env for DEV.' }));
          }

          const hasPrimedProp = Object.prototype.hasOwnProperty.call(g, 'primed');

          // If we have full cached state, reuse without any API call
          ...
          // Otherwise, resolve liveChatId with a single cheap call (URL → videoId → liveChatId)
          const liveChatId = g.liveChatId || (await getLiveChatIdFromUrl(TARGET_LIVESTREAM_URL));
          g.liveChatId = liveChatId;

          // Do NOT clear any cached token/primed here; preserve them if present
          res.send(
            devHtml({
              ok: true,
      ...
      app.post('/dev/prime', async (req, res) => {
        ...
        try {
          ...
          res.send(devHtml({ primed: g.primed, token, liveChatId: g.liveChatId }));
        } catch (e) {
          res.send(devHtml({ error: e.message || String(e) }));
        }
      });
      ...
      app.post('/dev/reset', async (req, res) => {
        resetDevState();
        res.send(devHtml({ reset: true, liveChatId: g.liveChatId, nextPageToken: g.nextPageToken, primed: g.primed }));
      });
    }
...
async function pollOnce(liveChatId) {
  ...
  try {
    ...
    // invalid page token error, need to prime again
    ...
    // some other error we dont handle yet. fail
    } else {
      throw err;
    }
  }
  ...
  return {
    ...
    nextDelaySuggestedMs:
      res.data.pollingIntervalMillis ?? Number(POLLING_FALLBACK_MS || 10000),
  };
}
